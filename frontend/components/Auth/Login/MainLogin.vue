        ...
        <div class="p-4 m-4 bg-gray-100 dark:bg-gray-800 rounded-md shadow-md w-[30rem]">
            <form @submit.prevent="sendLogin" class="flex flex-col items-center gap-3">
                ...
                <div class="w-full flex flex-col justify-start">
                    <label for="audience" class="w-full  text-[14px] mb-2">
                      Логин
                    </label>
                    <div class="input-container flex items-center border rounded-lg"
                        :class="{ 'p-invalid': Boolean(loginError.message) }">
                        <InputText v-model="loginForm.username" required id="username"
                ...
                <div class="w-full flex flex-col justify-start">
                    <label for="password" class="w-full  text-[14px] mb-2">
                      Пароль
                    </label>

                    <div class="input-container flex items-center border rounded-lg"
                        :class="{ 'p-invalid': Boolean(loginError.message) }">
                        <InputText :type="showPassword ? 'text' : 'password'" v-model="loginForm.password" required
                            id="password"
                            class="w-full bg-transparent border-none shadow-none h-[48px] focus:ring-0 focus:outline-none"
...
function sendLogin() {
  ...
  useNuxtApp()
    .$api.post(`api/admin/login`, formData)
    .then((response) => {
