<template>
    <div class="flex flex-col gap-4 py-4  items-center md:items-start">
      <span class="font-bold h-10 flex flex-col justify-end pb-2">{{ t('DateRangeFilter.heading') }}</span>
  
      <!-- Выбор диапазона дат -->
      <div class="flex items-center gap-4">
        <Calendar :disabled="disabled" v-model="dates" selectionMode="range" :manualInput="false" :placeholder="t('DateRangeFilter.placeholder')"
        ...
        <Button  icon="pi pi-times" class="p-button-rounded p-button-outlined p-button-sm" @click="clearDateFilter"
          :disabled="!dates || disabled" :aria-label="t('DateRangeFilter.clear')" />
      </div>
    </div>
  </template>
