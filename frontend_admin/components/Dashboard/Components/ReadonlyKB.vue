...
<template>
  ...
  <section v-for="(tree, sourceId) in sources" :key="sourceId" class="mb-4">
    ...
    <Panel :toggleable="true" :collapsed="true" class="mt-2 p-2 rounded border bg-gray-50 dark:bg-gray-800">
      <!-- ── Custom header (re-creates the old <summary>) -->
      <template #header>
        <!-- — Knowledge-base pseudo-source -->

        <div v-if="sourceId === 'kb'" class="flex flex-row justify-center items-center gap-2 cursor-pointer font-medium">
          <div
            :class="[
        ...
        <div v-else-if="isObjectId(sourceId)" class="flex items-center gap-2 cursor-pointer font-medium">
          <template v-if="contextMap[sourceId]">
            ...
            <div class="flex-shrink-0">
              ...
              >
                <i :class="[typeIcon(contextMap[sourceId].type), 'text-lg 2xl:text-xl']" />
              </div>
            </div>

            <!-- title + meta -->
            <div class="flex-1 flex flex-col justify-between">
              <p class="font-medium truncate text-[14px] 2xl:text-[17px] leading-5 max-w-[190px]">
                {{ contextMap[sourceId].title }}
              </p>
              <p class="text-[10px] text-slate-500 2xl:text-sm max-[375px]:hidden">
        ...
        <div v-else class="flex items-center gap-2 cursor-pointer font-medium">
          <span>{{ sourceId }}</span>
        </div>
      </template>
...
/* quick lookup by id */
const contextMap = computed(() => Object.fromEntries(contextList.value.map((c) => [c.id, c])));
