...
class UserAdmin(BaseAdmin):
    ...
    async def _hash_password_if_needed(
            self, data: dict, existing_hashed_password: Optional[str] = None) -> None:
    ...
    async def create(self, data: dict) -> dict:
        ...
        return await self._get_or_raise(result.inserted_id, "Failed to retrieve created object.")

    async def update(self, object_id: str, data: dict) -> dict:
        """Обновление пользователя с хэшированием пароля (если передан в незахэшированном виде) и проверкой уникальности."""
        ...
        if result.matched_count == 0:
            raise HTTPException(
                status_code=404,
                detail="Item not found for update.")

        return await self._get_or_raise(object_id, "Failed to retrieve updated object.")
