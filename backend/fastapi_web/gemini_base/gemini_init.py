...
class GeminiClient:
    ...
    async def chat_generate(
        ...
        Для каждого чанка вызываем  chat_generate_single, но НЕ записываем ответ
        ...
        user_text = self.extract_last_user_text(messages)
        total_input_words = self.count_words(user_text)
        logging.info(
        ...
        if total_input_words <= MAX_INPUT_TOKENS:
            ...
            single_resp = await self.chat_generate_single(model, messages, system_instruction, temperature)
            if 'error' in single_resp:
                logging.error(f"[GeminiClient] Ошибка {single_resp['detail']}")
            answer_text = self.extract_text(single_resp)
            answer_words = self.count_words(answer_text)
            logging.info(
                f"[GeminiClient] Single answer ~ {answer_words} words.")
            return self.convert_to_parts_format(single_resp)

        logging.info(
            "[GeminiClient] Input exceeds limit, splitting into chunks.")
        chunks = self.split_text(user_text, MAX_INPUT_TOKENS)
        logging.info(f"[GeminiClient] Got {len(chunks)} chunks.")

        for idx, ch in enumerate(chunks, start=1):
            ch_words = self.count_words(ch)
        ...
        for i, chunk_content in enumerate(chunks, start=1):
            ...
            part_resp = await self.chat_generate_single(model, chunk_messages, system_instruction, temperature)
            if "error" in part_resp:
                logging.error(
                    f"[GeminiClient] Error on chunk {i}: {part_resp['error']}")
                return part_resp

            chunk_answer_text = self.extract_text(part_resp)
            chunk_answer_words = self.count_words(chunk_answer_text)
            logging.info(
                f"[GeminiClient] Chunk {i} answer ~ {chunk_answer_words} words.")

            parsed_json = self.extract_json_from_resp(part_resp)
            if parsed_json is not None:
                logging.info(
                    f"[GeminiClient] Chunk {i} -> recognized JSON, merging.")
                self.merge_dicts(merged_json, parsed_json)
            else:
                logging.info(
                    f"[GeminiClient] Chunk {i} -> recognized text, appending.")
                merged_text = self.merge_texts(merged_text, chunk_answer_text)

        final_str = self.combine_final_output(merged_json, merged_text)
        logging.info(
    ...
    async def chat_generate_single(
        self,
        model: str,
        messages: List[Dict[str, Any]],
        system_instruction: Optional[str] = None,
        ...
        try:
            async with self.session.post(url, json=payload, headers=self.headers) as r:
                if r.status == 429:
                    ...
                    return await self.chat_generate_single(model, messages, temperature)
                if r.status != 200:
    ...
    def convert_to_parts_format(
        ...
        content_str = self.extract_text(original_resp)
        return {
            "candidates": [
                {
                    "content": {
    ...
    def extract_json_from_resp(
        ...
        """
        text_data = self.extract_text(resp)
    ...
    def merge_dicts(
        ...
        for k, v in source.items():
            if k in target and isinstance(
                ...
                self.merge_dicts(target[k], v)
            else:
                target[k] = v

    def merge_texts(self, acc: str, piece: str) -> str:
        """Добавляем piece к acc с переводом строки."""
        if not acc:
            return piece
        return acc + "\n" + piece

    def combine_final_output(self, merged_json: dict,
                             merged_text: str) -> str:
        """
