...
class GeminiClient:
    ...
    async def chat_generate(
        ...
        logging.info(f"[GeminiClient] chat_generate called. Last user message ~{total_input_words} words.")
        ...
        if total_input_words <= MAX_INPUT_TOKENS:
            ...
            logging.info(f"[GeminiClient] Single answer ~ {answer_words} words.")
            return self._convert_to_parts_format(single_resp)

        logging.info("[GeminiClient] Input exceeds limit, splitting into chunks.")
        ...
        for i, chunk_content in enumerate(chunks, start=1):
            logging.info(f"[GeminiClient] Processing chunk {i}/{len(chunks)}. History length: {len(history)}")
            ...
            if "error" in part_resp:
                logging.error(f"[GeminiClient] Error on chunk {i}: {part_resp['error']}")
                return part_resp

            chunk_answer_text = self._extract_text(part_resp)
            chunk_answer_words = self._count_words(chunk_answer_text)
            logging.info(f"[GeminiClient] Chunk {i} answer ~ {chunk_answer_words} words.")

            parsed_json = self._extract_json_from_resp(part_resp)
            if parsed_json is not None:
                logging.info(f"[GeminiClient] Chunk {i} -> recognized JSON, merging.")
                self._merge_dicts(merged_json, parsed_json)
            else:
                logging.info(f"[GeminiClient] Chunk {i} -> recognized text, appending.")
                merged_text = self._merge_texts(merged_text, chunk_answer_text)

        final_str = self._combine_final_output(merged_json, merged_text)
        logging.info("[GeminiClient] All chunks processed, returning unified answer.")
    ...
    async def _chat_generate_single(
        ...
        try:
            async with self.session.post(url, json=payload, headers=self.headers) as r:
                if r.status == 429:
                    logging.warning("[GeminiClient] Rate limited (429). Retrying in 10s...")
    ...
    def _convert_to_parts_format(self, original_resp: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _extract_json_from_resp(self, resp: Dict[str, Any]) -> Union[Dict[str, Any], None]:
        """
    ...
    def _combine_final_output(self, merged_json: dict, merged_text: str) -> str:
        """
