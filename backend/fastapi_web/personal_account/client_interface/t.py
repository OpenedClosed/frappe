...
from schemas.patient import (
    MainInfoSchema, ContactInfoSchema, AppointmentSchema, AccountVerificationEnum
)
...
class MainInfoAccount(CRMIntegrationMixin, BaseAccount):
    ...
    read_only_fields = ["patient_id", "account_status", "created_at", "updated_at"]
    ...
    async def update(self, object_id: str, data: dict, current_user=None):
        ...
        await self.patch_contacts_in_crm(
            obj.get("patient_id"),
            {k: normalize_numbers(v) if k == "phone" else v for k, v in data.items() if k in ("phone", "email")}
        )
...
class AppointmentAccount(BaseAccount):
    ...
    async def get_queryset(
        ...
        data = await get_client().future_appointments(
            main["patient_id"],
            from_date=main["created_at"].date() if "created_at" in main else date.today()
        )
        return [
            {
                "id": a["id"],
