...
def _is_client(role: Any) -> bool:
    """True, если роль = CLIENT (Enum или строка)."""
    if isinstance(role, SenderRole):
        return role == SenderRole.CLIENT
    # строка вида '{"en": "Client", "ru": "Клиент"}'
    return '"Client"' in (role or "")


def _is_ai(role: Any) -> bool:
    """True, если роль = AI Assistant (Enum или строка)."""
    if isinstance(role, SenderRole):
        return role == SenderRole.AI
    return '"AI Assistant"' in (role or "")
...
def to_constructor_message(msg: ChatMessage) -> dict:
    """
    Преобразует ChatMessage в формат MessageInput для Constructor.chat.

    • `isInbound=True`  → клиент.  
    • `sender`:
        - "Operator"      → консультант
        - "Auto Response" → auto-welcome
        - отсутствует     → AI-бот
    """
    base: dict[str, Any] = {
        "sentAt": int(msg.timestamp.replace(tzinfo=timezone.utc).timestamp() * 1000),
        "content": msg.message,
        "isInbound": _is_client(msg.sender_role),
    }

    # Расставляем подпись, если это не клиент и не AI
    if msg.metadata and msg.metadata.get("auto_response"):
        base["sender"] = "Auto Response"
    elif isinstance(msg.sender_role, SenderRole) and msg.sender_role == SenderRole.CONSULTANT:
        base["sender"] = "Operator"
    elif isinstance(msg.sender_role, str) and '"Consultant"' in msg.sender_role:
        base["sender"] = "Operator"
...
async def constructor_query(query: str, variables: dict):
    """Выполняет GraphQL-запрос к Constructor.chat; бросает RuntimeError при `errors`."""
    headers = {
        "Authorization": f"Bearer {settings.CONSTRUCTOR_TOKEN}",
        "Content-Type": "application/json",
        "Accept": "application/json",
...
async def create_constructor_chat(session: ChatSession):
    """
    Создаёт чат в Constructor.chat, синхронизирует **всю** историю
    и сохраняет `constructor_chat_id` в Mongo.
    """
    unsynced_all = [m for m in session.messages if not m.synced_to_constructor]
    ...
    # safety-check на коллизии
    existing = await mongo_db.chats.find_one(
        {"constructor_chat_id": chat_id, "chat_id": {"$ne": session.chat_id}}
    )
    if existing:
        logger.critical(
            "[Constructor] COLLISION: constructor_chat_id=%s уже назначен чату %s (конфликт с %s)",
    ...
    # помечаем историю как синхронизированную
    if unsynced_all:
        for m in unsynced_all:
            m.synced_to_constructor = True
...
async def import_messages_to_constructor(session: ChatSession, messages: List[ChatMessage]):
    """
    Универсальный эндпоинт **CreateChatMessages**:
    создаёт чат при необходимости и/или добавляет сообщения.
    Используем `externalChatId = session.chat_id` (уникален внутри ассистента).
    """
    await constructor_query(
        constants.IMPORT_CHAT_MESSAGES,
        {
            "assistantId": settings.CONSTRUCTOR_ASSISTANT_ID,
            "chatTitle": f"Chat {session.chat_id}",
            "externalChatId": session.chat_id,
            "messages": [to_constructor_message(m) for m in messages],
        },
    )
...
async def push_to_constructor(session: ChatSession, messages: List[ChatMessage]):
    """
    Отправляет **новые** (unsynced) сообщения в Constructor.chat.

    • Если ID чата ещё нет → создаём обычным путём.  
    • Если ID есть → createMessages.  
    • Параллельно оставляем возможность пользоваться универсальным import-методом
      (можно переключить одной строкой, если потребуется).
    """
    unsynced = [m for m in messages if not m.synced_to_constructor]
    if not unsynced:
        return

    # --- Вариант 1. Новый универсальный импорт ---
    await import_messages_to_constructor(session, unsynced)
    # ------------------------------------------------

    # --- Вариант 2. Текущая схема “создать-чат / добавить-сообщения” ---
    # if not session.constructor_chat_id:
    #     await create_constructor_chat(session)
    #     return
    ...
    # await mongo_db.chats.update_one(
    #     {"chat_id": session.chat_id},
    #     {"$set": {"messages.$[msg].synced_to_constructor": True}},
    #     array_filters=[{"msg.id": {"$in": [m.id for m in unsynced]}}],
    # )