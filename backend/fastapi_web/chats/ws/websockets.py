...
async def websocket_chat_endpoint(websocket: WebSocket, chat_id: str):
    ...
    id_to_connect = "admin_pass" if (is_superuser and (
        generated_id == client_id)) else generated_id if is_superuser else client_id
    ...
    try:
        while websocket.client_state == WebSocketState.CONNECTED:
            ...
            await handle_message(
                manager, typing_manager, data, chat_id, client_id, redis_session_key, redis_flood_key, is_superuser, user_language
            )
