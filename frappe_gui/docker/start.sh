...
# каталоги логов (иначе падают worker/schedule)
mkdir -p /workspace/logs || true
mkdir -p "${SITE_PATH}/logs" || true
touch "${SITE_PATH}/logs"/{frappe.log,database.log,web.log,worker.log,scheduler.log} || true
...
if [ ! -f "${SITE_CONFIG}" ]; then
  ...
  # common_site_config (REDIS_BASE нормализуем из REDIS_URL)
  python3 - <<'PY_PREP'
...
# Внутренний URL до FastAPI (server-to-server)
site_cfg["dantist_base_url"] = os.getenv("DANTIST_BASE_URL_INTERNAL", "http://backend:8000/api")

# iFrame: полный публичный URL скрытой старой админки + ORIGIN без пути
...
# ===== 4) Свой Procfile (без redis_* и без watch; без абсолютных путей node) =====
...
# ===== 5) Старт процессов (bench увидит наш Procfile) =====
exec bench start