...
# Версия: 2025-11-24 (auto-new-site-if-core-missing)
...
# ===== Создание сайта вынесено в функцию =====
create_site(){
  ...
  ok "Сайт создан (site_config и db_name/db_password сгенерированы Frappe)"
}
...
# ===== 2) создание сайта или пересоздание при отсутствии ядра =====
if [[ ! -f "$SITE_CFG" ]]; then
  create_site
else
  ...
  if core_tables_ok; then
    ...
    warn "База сайта неконсистентна (нет tabDefaultValue) — пересоздаю сайт ${SITE}"
    create_site
  fi
fi
...
# Быстрая диагностика (теперь только лог, без fatal)
quick_diag
...
# ===== 8) пароль Administrator (после того, как encryption_key уже подтянули из ENV) =====
do_admin_password
