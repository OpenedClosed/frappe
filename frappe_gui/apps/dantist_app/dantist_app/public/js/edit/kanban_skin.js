...
(() => {
  ...
  // ===== Performance helpers
  const PERF = (() => {
    let raf = 0; const jobs = new Set();
    const run = () => { raf = 0; const batch = Array.from(jobs); jobs.clear(); for (const fn of batch) { try { fn(); } catch(e){ console.warn("[DNT-PERF]", e); } } };
    const schedule = (fn) => { jobs.add(fn); if (!raf) raf = requestAnimationFrame(run); };
    const debounce = (fn, ms=80) => { let id=0; return (...a) => { clearTimeout(id); id=setTimeout(()=>fn(...a), ms); }; };
    return { schedule, debounce };
  })();

  const CALLQ = (() => {
    const MAX = 4; let active = 0; const q = [];
    const run = () => { while(active < MAX && q.length){ const job = q.shift(); active++; Promise.resolve().then(job.fn).then(job.res, job.rej).finally(()=>{ active--; run(); }); } };
    return (fn) => new Promise((res, rej)=>{ q.push({fn, res, rej}); run(); });
  })();
  const fcall = (args) => CALLQ(() => frappe.call(args));

  // ===== View flags & width persistence
  ...
  function resetColumnInlineWidth(col){ if (!col) return; col.style.removeProperty("--dnt-card-w"); col.style.minWidth = `calc(var(--dnt-card-w-default) + 24px)`; }
  function getColumnsEl(){ return Array.from(document.querySelectorAll(".kanban-column")); }
  function normalizeColumns(){ const mode = currentMode(); getColumnsEl().forEach(col=>{ const hasCards = !!col.querySelector(".kanban-card, .kanban-card-wrapper"); if (!hasCards){ resetColumnInlineWidth(col); sessionColW.delete(sessKey(col, mode)); } }); }
  function applyWidthsForMode(mode){ getColumnsEl().forEach(resetColumnInlineWidth); getColumnsEl().forEach(col=>{ const saved = loadColW(col, mode); const sessionW = getSessW(col, mode); const w = (sessionW ?? saved); if (w){ col.style.setProperty("--dnt-card-w", w + "px"); col.style.minWidth = `calc(${w}px + 24px)`; } }); normalizeColumns(); }
  function clearSavedColWidthsForBoard(board){ try{ const prefix = `dntKanbanColW::${board||"__"}::`; const keys = []; for (let i=0;i<localStorage.length;i++){ const k = localStorage.key(i); if (k && k.startsWith(prefix)) keys.push(k); } keys.forEach(k => localStorage.removeItem(k)); }catch{} }

  // ===== CSS (tuned: AssignTo centering; keep like visible)
  function injectCSS(){
    if(document.getElementById(CFG.cssId)) return;
    const s=document.createElement("style"); s.id=CFG.cssId;
    s.textContent = `
      :root{ --dnt-card-ch-compact:${CFG.compactChars}; --dnt-card-ch-comfy:${CFG.comfyChars}; --dnt-h-head-compact:36px; --dnt-h-head-comfy:52px; --dnt-assign-h-compact:22px; --dnt-assign-h-comfy:32px; --dnt-tasks-h-compact:72px; --dnt-tasks-h-comfy:100px; }
      ...
      html.${CFG.htmlClass} .kanban-card.content{ border-radius:14px; border:1px solid var(--border-color); background: var(--card-bg); padding:12px; box-shadow: var(--shadow-base, 0 1px 2px rgba(0,0,0,.06)); transition:transform .12s, box-shadow .12s; display:flex !important; flex-direction:column; gap:0; color: var(--text-color); width: var(--dnt-card-w); margin-inline:auto; }
      html.${CFG.htmlClass} .kanban-card.content:hover{ transform:translateY(-1px); box-shadow: var(--shadow-lg, 0 8px 22px rgba(0,0,0,.12)); }
      ...
      @media (prefers-reduced-motion: reduce){ html.${CFG.htmlClass} .dnt-softfade { transition: none !important; } }
      html.${CFG.htmlClass}.dnt-resizing *{ transition: none !important; }

      html.${CFG.htmlClass} .dnt-head{ display:flex; align-items:center; justify-content:space-between; gap:12px; min-width:0; margin-bottom:10px; min-height: var(--dnt-h-head-comfy); }
      ...
      html.${CFG.htmlClass} .kanban-image{ width:40px !important; height:40px !important; border-radius:10px; overflow:hidden; background: var(--bg-color); display:flex; align-items:center; justify-content:center; margin:0 !important; flex:0 0 40px; box-sizing:border-box; padding:2px; }
      ...
      html.${CFG.htmlClass} .kanban-card-doc .dnt-kv{ display:flex; align-items:center; gap:6px; background: var(--control-bg); border:1px solid var(--border-color); border-radius:10px; padding:4px 8px; font-size:12px; min-height:24px; color: var(--text-color); white-space:nowrap; overflow:visible; text-overflow:clip; }
      ...
      /* AssignTo row — centered vertically */
      html.${CFG.htmlClass} .dnt-assign-slot{
        height: var(--dnt-assign-h-comfy); min-height: var(--dnt-assign-h-comfy); max-height: var(--dnt-assign-h-comfy);
        display:flex; align-items:center; justify-content:space-between; gap:8px; overflow:hidden; padding-block:2px; line-height:1.2;
        margin-top:8px;
      }
      html.${CFG.htmlClass}.dnt-compact-on .dnt-assign-slot{
        height: var(--dnt-assign-h-compact); min-height: var(--dnt-assign-h-compact); max-height: var(--dnt-assign-h-compact);
      }
      ...
      html.${CFG.htmlClass} .document-star, html.${CFG.htmlClass} .star-action, html.${CFG.htmlClass} .favorite-action{ display:none !important; }
      html.${CFG.htmlClass} .dnt-assign-right .like-action, html.${CFG.htmlClass} .dnt-assign-right [data-action="like"], html.${CFG.htmlClass} .dnt-assign-right .btn-like{ display:inline-flex !important; align-items:center; gap:4px; opacity:1 !important; visibility:visible !important; cursor:pointer; }
      ...
      html.${CFG.htmlClass} .kanban-card-meta, html.${CFG.htmlClass} .kanban-card-doc, html.${CFG.htmlClass} .dnt-assign-slot, html.${CFG.htmlClass} .dnt-tasks-mini{ -webkit-user-drag: none; user-select: none; cursor: grab; }
      ...
      html.${CFG.htmlClass} .dnt-resizer{ position:absolute; right:4px; bottom:4px; width:16px; height:16px; cursor:nwse-resize; opacity:.9; border-radius:4px; display:flex; align-items:center; justify-content:center; color: var(--border-color); background: transparent; z-index:6; }
      ...
      html.${CFG.htmlClass} .dnt-skel-line{ height:12px; border-radius:6px; background: linear-gradient(90deg, var(--control-bg) 25%, color-mix(in oklab, var(--control-bg) 70%, white) 50%, var(--control-bg) 75%); background-size:200% 100%; animation:dnt-shimmer 1.1s linear infinite; }
      html.${CFG.htmlClass} .dnt-skel-line.h16{ height:16px; border-radius:8px; }
      html.${CFG.htmlClass} .w20{ width:20%; } .w30{ width:30%; } .w40{ width:40%; } .w50{ width:50%; } .w60{ width:60%; } .w70{ width:70%; } .w80{ width:80%; } .w100{ width:100%; }
  ...
  function buildMetaMaps(doctype){
    ...
    (frappe.model.std_fields||[]).forEach(f=>{ const lbl = CLEAN(f?.label||""); if (lbl){ label2fn[LBL_KEY(lbl)] = f.fieldname; fn2label[f.fieldname] = f.fieldname in fn2label ? fn2label[f.fieldname] : lbl; fn2df[f.fieldname] = fn2df[f.fieldname] || f; }});
  ...
  // ===== Field/doc cache (in-memory + localStorage hydration)
  const DOC_CACHE = new Map();
  const docKey = (dt, name) => `${dt}::${name}`;

  const DOC_LS = (()=> {
    const prefix = "dntDocCache::";
    const indexKey = (dt)=> `dntDocCacheIndex::${dt}`;
    const MAX = 300;

    function read(dt, name){
      const raw = ls_get(prefix + dt + "::" + name);
      if (!raw) return null;
      try { return JSON.parse(raw); } catch { return null; }
    }
    function write(dt, name, obj){
      try {
        const key = prefix + dt + "::" + name;
        ls_set(key, JSON.stringify(obj));
        const ik = indexKey(dt);
        const raw = ls_get(ik);
        let arr = [];
        try { arr = raw ? JSON.parse(raw) : []; } catch { arr = []; }
        const set = new Set(arr);
        set.delete(name); set.add(name);
        while (set.size > MAX) {
          const first = set.values().next().value;
          set.delete(first);
          ls_del(prefix + dt + "::" + first);
        }
        ls_set(ik, JSON.stringify(Array.from(set)));
      } catch {}
    }
    function hydrate(dt, names){
      names.forEach(n => {
        if (DOC_CACHE.has(docKey(dt, n))) return;
        const obj = read(dt, n);
        if (obj && typeof obj === "object") DOC_CACHE.set(docKey(dt, n), obj);
      });
    }
    return { read, write, hydrate };
  })();
  ...
  function mergeDocCache(dt, name, obj){
    ...
    // persist to LS (keep __modified if present)
    DOC_LS.write(dt, name, next);
    return next;
  }
  ...
  function normalizeDocFields(docEl, ctx, opts={}){
    ...
    if (!missing.length){
      if (docEl.dataset.dntCardHash !== card_hash_if_ready){
        ...
        docEl.innerHTML = ""; docEl.appendChild(frag); ensure_dashes(docEl);
  ...
  async function backfillAll(container, ctx, labelsOn, orderMap){
    ...
    Array.from(container.querySelectorAll(":scope > .dnt-kv")).forEach(ch=>{
      const lbl = CLEAN(ch.dataset.dntLabel || ch.querySelector(".dnt-k")?.textContent || "").replace(/:$/,''); if (!lbl) return;
    ...
    Array.from(container.querySelectorAll(":scope > .dnt-kv")).forEach(ch=>{
      const lbl = CLEAN(ch.dataset.dntLabel || ch.querySelector(".dnt-k")?.textContent || "").replace(/:$/,''); if (lbl) return;
      const val = CLEAN(ch.querySelector(".dnt-v")?.textContent || ""); if (!val) return;
      if (!valueIndex.has(val)) valueIndex.set(val, []); valueIndex.get(val).push(ch);
    });
  ...
  function miniHtml(rows, total, caseName){
    ...
    const lines = rows.map(ti=>{
      ...
      const ttl  = frappe.utils.escape_html(plain_text(ti.description || ti.name));
  ...
  async function tasks_version(caseName){
    const cntResp = await fcall({ method: "frappe.client.get_count", args: { doctype: "ToDo", filters: { reference_type: CFG.caseDoctype, reference_name: caseName, status: "Open" } } });
    const lastTargetResp = await fcall({ method: "frappe.client.get_list", args: { doctype: "ToDo", filters: { reference_type: CFG.caseDoctype, reference_name: caseName, status: "Open", custom_target_datetime: ["is", "set"] }, fields: ["custom_target_datetime"], limit_page_length: 1, order_by: "custom_target_datetime desc" } });
  ...
  async function fetchMiniPrimary(caseName){
    const { message } = await fcall({ method: CFG.tasksMethod, args: { name: caseName, status: "Open", limit_start: 0, limit_page_length: CFG.tasksLimit, order: "desc" } });
    const rows  = (message && message.rows) || []; const total = (message && message.total) || rows.length; return { rows, total };
  }
  async function fetchMiniFallback(caseName){
    try{
      const { message: rows=[] } = await fcall({ method: "frappe.client.get_list", args: { doctype: "ToDo", filters: { reference_type: CFG.caseDoctype, reference_name: caseName, status: "Open" }, fields: ["name","description","status","date","custom_due_datetime","custom_target_datetime","modified"], limit_page_length: CFG.tasksLimit, order_by: "modified desc" } });
      return { rows, total: rows.length };
    } catch { return { rows: [], total: 0 }; }
  }

  async function loadMini(container, caseName, opts={}){
    const soft = !!opts.soft; const lsHtmlKey = `dntMiniHtml::${caseName}`; const lsMetaKey = `dntMiniMeta::${caseName}`; const metaPrev = miniCacheMeta.get(caseName);
    try{
      if (!soft) showTasksSkeleton(container);
      const metaNow = await tasks_version(caseName);
      const lsMetaRaw = ls_get(lsMetaKey); const lsHtml = ls_get(lsHtmlKey); let lsMeta = null; try{ lsMeta = lsMetaRaw ? JSON.parse(lsMetaRaw) : null; }catch{ lsMeta = null; }
      if (lsMeta && lsMeta.versionKey === metaNow.versionKey && lsHtml){
        miniCacheHtml.set(caseName, lsHtml); miniCacheMeta.set(caseName, { versionKey: metaNow.versionKey });
        container.innerHTML = lsHtml; bindMini(container, caseName); container.dataset.dntMiniInit = "1";
      ...
      if (metaPrev && metaPrev.versionKey === metaNow.versionKey && miniCacheHtml.has(caseName)){
        ...
        container.innerHTML = html; bindMini(container, caseName); container.dataset.dntMiniInit = "1";
        ls_set(lsHtmlKey, html); ls_set(lsMetaKey, JSON.stringify({ versionKey: metaNow.versionKey }));
      ...
      miniCacheHtml.set(caseName, html); miniCacheMeta.set(caseName, { versionKey: metaNow.versionKey });
      ls_set(lsHtmlKey, html); ls_set(lsMetaKey, JSON.stringify({ versionKey: metaNow.versionKey }));
      const unlock = lockCardHeight(container); container.innerHTML = html; bindMini(container, caseName); container.dataset.dntMiniInit = "1"; unlock();
      dbg(metaPrev ? "♻️ Tasks refreshed" : "🆕 Tasks first load", caseName);
      container.dispatchEvent(new CustomEvent("dnt:tasks-version", { detail: { versionKey: metaNow.versionKey }}));
    } catch {
      if (miniCacheHtml.has(caseName)){
        const unlock = lockCardHeight(container); const html = miniCacheHtml.get(caseName);
        container.innerHTML = html; bindMini(container, caseName); container.dataset.dntMiniInit = "1"; unlock();
      ...
      const lsHtml = ls_get(lsHtmlKey); const lsMetaRaw = ls_get(lsMetaKey);
      if (lsHtml && lsMetaRaw){
        let parsed=null; try{ parsed = JSON.parse(lsMetaRaw); }catch{}
        miniCacheHtml.set(caseName, lsHtml); if (parsed?.versionKey) miniCacheMeta.set(caseName, { versionKey: parsed.versionKey });
        const unlock = lockCardHeight(container); container.innerHTML = lsHtml; bindMini(container, caseName); container.dataset.dntMiniInit = "1"; unlock();
  ...
  // ===== Lazy observe for cards (strictly lazy upgrade/normalize)
  ...
  function observeForLazy(wrap){ const io = ensureLazyIO(); if (io) io.observe(wrap); }
  ...
  function getCardColumnValue(wrap){ return wrap?.closest(".kanban-column")?.getAttribute("data-column-value") || ""; }
  ...
  function attachResizer(wrapper){
    ...
    const col = wrapper.closest(".kanban-column"); if (!card || !col) return;
    const handle = document.createElement("div"); handle.className = "dnt-resizer"; handle.title = t("Resize width"); handle.innerHTML = ICONS.resizerGrip; wrapper.appendChild(handle);
    let currentW = null;
    const startResize = (evStart)=>{
      ...
      const startX = evStart.clientX; const startRect = card.getBoundingClientRect(); const mode = currentMode();
  ...
  function upgradeCard(wrapper){
    const card = wrapper?.querySelector?.(".kanban-card.content, .kanban-card"); if(!card || card.dataset.dntUpgraded==="1") return;
    ...
    [meta, doc, body.querySelector(".dnt-assign-slot"), body.querySelector(".dnt-tasks-mini")].filter(Boolean).forEach(n => n.classList.add("dnt-softfade"));
    ...
    const assignments = (meta || body).querySelector(".kanban-assignments"); if (assignments && assignments.parentElement !== assignLeft) assignLeft.appendChild(assignments);
    ...
    if (!wrapper.querySelector(".dnt-card-actions")){
      ...
      const bOpen = document.createElement("div"); bOpen.className="dnt-icon-btn"; bOpen.title=t("Open"); bOpen.innerHTML = frappe.utils.icon("external-link","sm");
  ...
  function headerOfColumn(col){ return col.querySelector(".kanban-column-title, .kanban-title, .kanban-title-area, .kanban-column .title, .kanban-card-title-area") || null; }
  function countCards(col){ return col.querySelectorAll(".kanban-card-wrapper, .kanban-cards > .kanban-card").length; }
  function updateColumnCounts(){ getColumnsEl().forEach(col=>{ const head = headerOfColumn(col); if (!head) return; let badge = head.querySelector(".dnt-col-count"); const n = countCards(col); if (!badge){ badge = document.createElement("span"); badge.className = "dnt-col-count"; head.appendChild(badge); } badge.textContent = ` (${n})`; }); }

  // ===== Header controls
  function findSettingsAnchor(){ return ( document.querySelector(".page-actions .page-icon-group") || document.querySelector(".page-actions") || document.querySelector(".standard-actions .page-icon-group") || document.querySelector(".standard-actions") || document.querySelector(".page-head-content") || document.querySelector(".page-title") ); }
  ...
  function routeToListWithBoardFilter(){
    const dt = getDoctype(); const boardTitle = getBoardName(); if (!dt) return;
  ...
  function buildSettingsDropdown(){
    const wrap = document.createElement("div"); wrap.id = CFG.settingsBtnId; wrap.className = "btn-group btn-group-sm";
    const btn = document.createElement("button"); btn.className = "btn btn-default icon-btn btn-sm dropdown-toggle"; btn.setAttribute("data-toggle","dropdown"); btn.setAttribute("aria-expanded","false"); btn.setAttribute("title", t("Kanban options"));
    btn.innerHTML = ICONS.settings;
    wrap.appendChild(btn);

    const menu = document.createElement("ul"); menu.className = "dropdown-menu";
    menu.innerHTML = `<li><a class="grey-link dropdown-item dnt-open-board-settings" href="#" onclick="return false;">${frappe.utils.icon("edit","sm")} <span class="menu-item-label">${t("Board Settings")}</span></a></li><li class="dropdown-divider"></li><li><a class="grey-link dropdown-item dnt-toggle-labels" href="#" onclick="return false;">${frappe.utils.icon("tag","sm")} <span class="menu-item-label dnt-toggle-labels-text"></span></a></li>`;
    ...
    menu.querySelector(".dnt-toggle-labels").addEventListener("click", async (e)=>{
      ...
      try{
        ...
        refreshLabelsText(); refreshVisibleCardsDocs();
        ...
        btn.addEventListener("click", ()=>{
          setTimeout(()=>{
            ...
            Array.from(menu.querySelectorAll("li.user-action a.dropdown-item")).forEach(a=>{
              ...
              a.onclick = (e)=>{ e.preventDefault(); e.stopPropagation(); const dt = getDoctype(); frappe.set_route("List", dt, "Kanban", boardTitle); };
            });
          }, 0);
        });
      }
  ...
  const board_root = () => document.querySelector(".kanban-board") || document;
  const qsa_in = (root, sel) => Array.from((root || document).querySelectorAll(sel));

  // ===== Batched values with cache (FIX: return merged values)
  async function getValuesBatch(doctype, name, fields) {
    ...
    if (missing.length) {
      try {
        const { message: fetched = {} } = await fcall({
  ...
  const refreshVisibleCardsDocs_bounced = PERF.debounce(() => {
    const root = board_root();
  ...
  function attach_main_observer(){
    if (window.__dntKanbanMO) { try { window.__dntKanbanMO.disconnect(); } catch {} }
    const root = board_root();
    const mo = new MutationObserver((muts)=>{
      if(!isKanbanRoute()) return;
      const touchedCards = new Set(); const touchedColumns = new Set();
      const collect = (node) => {
  ...
  // ==== Visible-first field cache prime + LS hydration
  function visible_wrappers(){
    const root = board_root();
    const nodes = qsa_in(root, ".kanban-card-wrapper, .kanban-column .kanban-card");
    return nodes.map(el => el.classList?.contains?.("kanban-card") ? (el.closest(".kanban-card-wrapper") || el) : el);
  }
  ...
  async function prime_doc_fields_cache_for_visible_cards(){
    ...
    const wraps = visible_wrappers();
    const vis = wraps.filter(isWrapVisible);
    ...
    let versionRows = [];
    try{
      const { message: rows = [] } = await fcall({ method: "frappe.client.get_list", args: { doctype: dt, filters: [["name","in", unique]], fields: ["name","modified"], limit_page_length: unique.length } });
      versionRows = rows;
    }catch{}
    ...
    try{
      const { message: rows = [] } = await fcall({
        method: "frappe.client.get_list",
        args: { doctype: dt, filters: [["name","in", Array.from(new Set(changed))]], fields: ["name","modified", ...boardFields, "first_name","middle_name","last_name","title"], limit_page_length: changed.length }
      });
      rows.forEach(r => {
  ...
  function injectControls(){
    ...
    const wrap = document.createElement("div"); wrap.id = CFG.modeToggleId; wrap.className = "btn-group btn-group-sm dnt-mode-toggle";
    ...
    const mo = new MutationObserver(()=> {
      ...
      exposeSelectKanban(); hideViewSwitcher();
    });
  ...
  async function run(){
    ...
    try{
      ...
      const names = wraps.map(el => el.getAttribute("data-name") || el.dataset?.name || el.querySelector?.(".kanban-card")?.getAttribute?.("data-name")).filter(Boolean);
      DOC_LS.hydrate(dt, Array.from(new Set(names)));
    }catch{}
